...
resource "aws_nat_gateway" "nat_gateway_az1" {
  ...
  tags   = {
    Name = "${project_name}-${environment}-nat-gateway-az1"
    Developer = "Terraform"
  }
...
resource "aws_nat_gateway" "nat_gateway_az2" {
  ...
  tags   = {
    Name = "${project_name}-${environment}-nat-gateway-az2"
    Developer = "Terraform"
  }
...
resource "aws_route_table" "private_route_table_az1" {
  ...
  tags   = {
    Name = "${project_name}-${environment}-private-route-table-az1"
  }
}
...
resource "aws_route_table" "private_route_table_az2" {
  ...
  tags   = {
    Name = "${project_name}-${environment}-private-route-table-az2"
  }
}
